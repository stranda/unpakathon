---
title: "March 2016 unpakathon preliminary results"
author: "A Strand"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Work done leading up to March meeting}
...
```{r,fig.width=9}
exp1long <- phenolong.clipped %>% filter(meta.experiment=="1",inrange) %>% group_by(variable,accession) %>% summarise(value=mean(value,na.rm=T)) 
...
Change phenolong.clipped to phenolong in the next chunk to take all the data
```{r,fig.width=9}
exp2long <- phenolong.clipped %>% filter(meta.experiment=="2",inrange) %>% group_by(variable,accession) %>% summarise(value=mean(value,na.rm=T)) 
...
#phenos=c(unique(exp1long$variable))
phenos<- c("aborted.fruits","avg.fruit.length","basal.branch","days.to.bolt","diameter.at.bolt","fruitnum","germinants.14d","inflorescence.height", "ttl.maininfl.branch")
exp2long <- exp2long %>% filter(variable %in% c(phenos,"experiment"))
